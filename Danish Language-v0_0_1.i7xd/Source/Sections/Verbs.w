...
[ [ TODO - figure out negations ]
In Danish er ikke is a verb meaning to aren’t.
In Danish kan ikke is a verb meaning to can't.
In Danish gør ikke is a verb meaning to don't.
In Danish har ikke is a verb meaning to haven't.
In Danish må ikke is a verb meaning to mustn't.
In Danish måtte ikke is a verb meaning to mightn't.
In Danish må ikke is a verb meaning to mayn't.
In Danish ville ikke is a verb meaning to wouldn't.
In Danish kunne ikke is a verb meaning to couldn't.
In Danish burde ikke is a verb meaning to shouldn't.
In Danish vil ikke is a verb meaning to won't.
]
...
In Danish tabe is a verb meaning to drop.
...
In Danish lede is a verb meaning to lead.
In Danish kunne lide is a verb meaning to like.
...
In Danish vælge is a verb meaning to pick.
...
In Danish synes is a verb meaning to seem.
...
In Danish vinke is a verb meaning to wave.
...
This syntax is tailored for English. In English, the present participle is more common (he is waiting),
while it's rarely used in Danish. A bigger problem is that regular verb forms in English are more often
identical to the infinitive (to wait, I wait), except for the second person singular present (he waits).
This means that if the author doesn't supply the present plural form, for example, it's set to be the
same as the infinitive. This works for English (to wait, they wait), but rarely works well for Danish
(at vente, de vente), since it obviously should be the same as the other present tenses (han venter, de venter).
For that reason, all tenses should probably be explicitly defined even though Danish doesn't conjugate verbs
for plurality.

Note that this syntax is deprecated, because it generalises badly to other languages than English (as
demonstrated), and the source text is only allowed to define some forms of the verb (for example, there's no way
to define the imperative form). Still, we might as well support it, in case the author wants to make up a
verb with very irregular forms.
...
For the general Danish verbs you'll find in a dictionary, we'll write a producing
Preform grammar which can construct all the forms automatically.
...
Inform reserves the first verb form for the infinitive, which is fine for Danish.

By default, Inform uses this as the base form of the verb as well ("form type 0"), but it doesn't need to be identical to the infinitive.
The imperative (see below) could potentially have worked better as the base form; in Danish the imperative form of a verb is usually its stem,
and the infinitive is the stem/imperative form plus an -e suffix. We'll still use the infinitive as the base form, but we'll define it
...
Inform reserves the second verb form for the present participle. Present participle is common in English (as what they,
somewhat incorrectly linguistically speaking, call "gerund": "playing on computers is fun"), which isn't used in Danish,
and as the finite progressive/continuous form ("they are always playing"), which is rarely used in Danish. However, the
present participle is used adjectively in Danish ("the playing children", "de lekende børn"), so it's still useful
for us here.

However, English conflates the gerund and the present participle. Inform calls this form present participle, but it uses it
as the gerund when defining verbs as actions:

>> Smoothing is an action applying to one thing.

Here we obviously can't use the Danish present participle, we have to use the verbal/gerundian noun (nomen actionis), as it
is a verb acting as a noun. But Inform doesn't have a concept of the gerund and the present participle being different,
...
perfect ("har sunget") and the past pluperfect (plusquamperfect) ("havde sunget").

@h Adjoint infinitive (form type 4).

Inform reserves the fourth verb form for what it calls the "adjoint infinitive", which
is when a modal verb construction is used and the second verb is in the
infinitive ("be able to touch").

@h Present tense (form type 5).

Just like English Inform does, we define the fifth verb form to be the present tense.
...
And also just like English Inform, the sixth verb form is the past tense (preterite/imperfect).
...
In English, the imperative form is identical to the infinitive form, but in Danish it's different.
We therefore define the seventh verb form as the imperative.

In Danish the imperative is finite, and it can't be conjugated. (Archaic Danish had a plural
imperative, and also a passive imperative, but these are no longer used and we don't bother
implementing them.) It's therefore not useful for adaptive text, ie. it can't be adapted to the
story tense and story viewpoint. However, an author might still want to adapt verbs to the imperative
manually, especially because the player is likely to write all their commands in the imperative.
=
Imperative tense is a grammatical tense.

To say imperative mood/-- of (V - a verb):
	(- {V}(7); -).

@h Verb form type definitions.

As stated, Inform also reserves verb form type zero as the base form, but it will be identical to the
infinitive (although that's not required to be the case). We then end up with these forms:
...
(1) Infinitive
(2) Present participle (præsens partisipp) (syngende) - brukes kun som adjektiv på dansk!
...
created by //Dansk Sprognævn -> https://dsn.dk//, which contains 7493 Danish verbs.
